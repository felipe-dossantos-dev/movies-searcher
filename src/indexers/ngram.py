...
class NGramIndexer(Indexer):
    def __init__(self, index_dir: Path, rebuild: bool = False, ngram_size: int = 3):
    ...
    def build_index(self, data_dir: Path) -> None:
        ...
        for file_path in data_dir.glob("*.txt"):
            with open(file_path, "r") as f:
                # melhorar isso daqui depois para salvar conforme vai buscando
                content = f.read()
        ...
        for ngram, word_paths in self.ngram_index.items():
            ngram_file_path = self.index_dir / f"{ngram}.ngram.pickle"
    ...
    def get_all_words(self) -> Set[Word]:
        for file_path in self.index_dir.glob("*.ngram.pickle"):
